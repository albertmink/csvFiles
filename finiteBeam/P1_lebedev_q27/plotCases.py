...
for i in range(1,36):
    ...
    dfMacro['p1_q15_lebedev'] = getDataValues('./../P1_dir_lebedev_q15/100case'+str(i)+'.csv', line)[:-2]
    dfMacro['p1_q27_lebedev'] = getDataValues('100case'+str(i)+'.csv', line)[:-2]
    dfMacro['MC'] = getDataValues('../lena/lena'+str(i)+'.csv', line)[:-2]
    dfMacro.plot()
    plt.yscale('log')
    plt.savefig('./q6_q14_q26_case'+str(i)+'.png')
    plt.close()
